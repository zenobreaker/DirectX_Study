...
const int CUserInterface::VertexCount = 6;
...
void CUserInterface::Tick()
{
	static float y = 0.5f; 
	ImGui::SliderFloat("Position Y", &y, -1.0f, 1.0f); 

	Verticies[1].Position.Y = y; ;

	CD3D::Get()->GetDeviceContext()->UpdateSubresource(VertexBuffer, 0, nullptr, Verticies, sizeof(FVertex) * VertexCount, 0);
}
