...
CTextureBuffer::CTextureBuffer(ID3D11Texture2D* InSource)
{
	...
	CD3D::Get()->GetDeviceContext()->CopyResource(texture, InSource); // 미리 복사해놓음 
...
void CTextureBuffer::CreateSRV()
{
	...
	D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
	ZeroMemory(&srvDesc, sizeof(D3D11_SHADER_RESOURCE_VIEW_DESC));
	srvDesc.Format = desc.Format;  // 텍스처는 모르니까 정의된 내용으로 
	srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2DARRAY; // 버퍼의 확장명으로 
...
void CTextureBuffer::CreateOutput()
{
	ID3D11Texture2D* texture = nullptr;
	...
	Check(CD3D::Get()->GetDevice()->CreateTexture2D(
		&desc,
		nullptr,
		&texture));
...
void CTextureBuffer::CreateUAV()
{
	...
	Check(CD3D::Get()->GetDevice()->CreateUnorderedAccessView(texture, &uavDesc, &UAV) >= 0);
}

void CTextureBuffer::CreateResult()
{
	ID3D11Texture2D* texture = (ID3D11Texture2D*)Output;
	...
	Check(CD3D::Get()->GetDevice()->CreateShaderResourceView(texture, &srvDesc, &OutputSRV));
}


//------------------------------------------------------------------------------

CStructuredBuffer::CStructuredBuffer(void* InInputData, UINT InInputStride, UINT InInputCount, UINT InOutputStride, UINT InOutputCount)
	: InputData(InInputData)
	, InputCount(InInputCount), InputStride(InInputStride)
	, OutputStride(InOutputStride), OutputCount(InOutputCount)
{
	if (InOutputStride == 0 || InOutputCount == 0)
	{
		this->OutputStride = InInputStride;
		this->OutputCount = InInputCount;
	}
...
void CStructuredBuffer::CreateSRV()
{
	...
	D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
	ZeroMemory(&srvDesc, sizeof(D3D11_SHADER_RESOURCE_VIEW_DESC));
	srvDesc.Format = DXGI_FORMAT_UNKNOWN;  // 셰이더에서 편한대로 끊어서 사용을 의미
	srvDesc.ViewDimension = D3D11_SRV_DIMENSION_BUFFEREX; // 버퍼의 확장명으로 
	srvDesc.BufferEx.NumElements = InputCount; // 실제 개수 (바이트 수로나눈 값)
...
void CStructuredBuffer::CreateOutput()
{
	...
	desc.BindFlags = D3D11_BIND_UNORDERED_ACCESS; // UAV에 연결되므로 플래그 변경
	desc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED; // 어떠한 버퍼의 형태를 결정
	desc.Usage = D3D11_USAGE_DEFAULT; // GPU -> CPU
...
void CStructuredBuffer::CreateUAV()
{
	...
	D3D11_UNORDERED_ACCESS_VIEW_DESC uavDesc;
	ZeroMemory(&uavDesc, sizeof(D3D11_UNORDERED_ACCESS_VIEW_DESC));
	uavDesc.Format = DXGI_FORMAT_UNKNOWN; // 타입을 모르지만 4바이트(32비트) 
	uavDesc.ViewDimension = D3D11_UAV_DIMENSION_BUFFER;
	uavDesc.Buffer.NumElements = OutputCount; // 실제 개수 (바이트 수로나눈 값)
...
void CStructuredBuffer::SetInputData(void* InData)
{
	D3D11_MAPPED_SUBRESOURCE subResource;
	// 이미 맵이 걸려 있는지 검사
	Check(CD3D::Get()->GetDeviceContext()->Map(Input, 0, D3D11_MAP_WRITE_DISCARD, 0, &subResource));
	{
		memcpy(subResource.pData, InData, GetInputByteWidth());
	}
	CD3D::Get()->GetDeviceContext()->Unmap(Input, 0);
}

void CStructuredBuffer::GetOutputData(void* OutData)
{
	CD3D::Get()->GetDeviceContext()->CopyResource(Result, Output);
	...
	Check(CD3D::Get()->GetDeviceContext()->Map(Result, 0, D3D11_MAP_READ, 0, &subResource));
	{
		memcpy(OutData, subResource.pData, GetOutputByteWidth());
	}
	CD3D::Get()->GetDeviceContext()->Unmap(Result, 0);
}
